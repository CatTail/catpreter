...
var RegExpLexer = (function () {
  ...
  /**
   * CATTAIL
   * Expend macro, deleminate start-conditions in rules and mapping rule action from
   * string to number representation.
   *
   * @param {array.<array|string>} rules
   * @param {object.<string, string>} macros
   * @param {array.<string>} actions Array of string that will be the ouput of result lexer
   * @param {object.<string, number>} tokens Terminals generated by catpreter.Parser
   * @param {object.<string, object>} startConditions Alias for RegExpLexer.conditions
   * @param {boolean} careless
   */
  ...
  function prepareRules(rules, macros, actions, tokens, startConditions, caseless) {
    ...
    for (i=0;i < rules.length; i++) {
      // CATTAIL: identify rules with no start condition
      if (Object.prototype.toString.apply(rules[i][0]) !== '[object Array]') {
        // CATTAIL: inclusive start condition also including rules without start condition
        ...
        // CATTAIL: special start condition specifier <*> match all start condition
      ...
      if (typeof rules[i][1] === 'function') {
        // CATTAIL: remove funciton wrapper if action is a `function`
  ...
  function prepareMacros (macros) {
    var cont = true,
    m,i,k,mnew;
    // CATTAIL: macro being replace may also contain macro, so we need
    // a extra loop here.
    while (cont) {
      cont = false;
  ...
  /**
   * CATTAIL
   * Refer [[http://dinosaur.compilertools.net/flex/flex_11.html#SEC11]] to find more about start condition.
   *
   * @param {object.<string, object>|null} conditions
   */
  ...
  /**
   * CATTAIL
   * @param {object.<string, *> } dict
   * @param {object.<string, string>} tokens
   * @return {function(object, object, number, string)} Function build by `Function`
   */
  function buildActions (dict, tokens) {
    // CATTAIL: @type {string} dict.actionInclude
    // YYSTATE is used in start condition, it represente the current start condition.
    var actions = [dict.actionInclude || '', "var YYSTATE=YY_START;"];
    var tok;
    var toks = {};

    // CATTAIL: reverse tokens' key and value
    for (tok in tokens) {
      toks[tokens[tok]] = tok;
    }
  ...
  function RegExpLexer (dict, input, tokens) {
    if (typeof dict === 'string') {
      dict = require("./catlex").parse(dict);
    }
    dict = dict || {};
  ...
  RegExpLexer.prototype = {
    ...
    /**
     * CATTAIL
     * Read one character at a time, increasing conresponding data.
     */
    ...
    generateModule: function generateModule(opt) {
      opt = opt || {};
      var out = "/* catpreter generated lexer */",
    ...
    generateCommonJSModule: function generateCommonJSModule(opt) {
      opt = opt || {};
      var out = "/* catpreter generated lexer as commonjs module */",
      moduleName = opt.moduleName || "lexer";
