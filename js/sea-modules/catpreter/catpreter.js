...
if (typeof console !== 'undefined' && console.log) {
  Catpreter.print = console.log;
} else if (typeof puts !== 'undefined') {
  Catpreter.print = function print () { puts([].join.call(arguments, ' ')); };
} else if (typeof print !== 'undefined') {
  Catpreter.print = print;
} else {
  Catpreter.print = function print () {};
}

Catpreter.Parser = (function () {
  ...
  var Production = typal.construct({
    /**
     * @constructor
     * @param {string} symbol Nonterminal
     * @param {array} handle
     * @param {number} id Production index in this.productions
     */
  ...
  /**
   * CATTAIL
   * The generator object will be a prototype of lr*, ll*,
   * Using other technology such as lookahead to build fully functioned parser.
   * @param {object} grammar
   * @param {object} opt
   */
  generator.constructor = function Catpreter_Generator (grammar, opt) {
    if (typeof grammar === 'string') {
      grammar = require("./bnf").parse(grammar);
    }
  ...
  generator.processGrammar = function processGrammarDef (grammar) {
    ...
    // CATTAIL: transform ebnf rules
    if (!grammar.bnf && grammar.ebnf) {
      bnf = grammar.bnf = require("./ebnf").transform(grammar.ebnf);
    }
  ...
  /**
   * Augment grammar by add $accept and $end symbol.
   * @param {object} grammar
   */
  ...
  /**
   * CATTAIL
   * Transform operator representation from array to object mapping.
   *
   * @param {array.<array.<string>>} ops Array of operators
   ...
   * @param {object.<string, Nonterminal>} nonterminals
   * @param {array} symbols
  ...
  generator.buildProductions = function buildProductions(bnf, productions, nonterminals, symbols, operators) {
    ...
    var prods, symbol;
    var productions_ = [0]; // CATTAIL: production number id, handle length map
    var symbolId = 1;
    var symbols_ = {}; // CATTAIL: symbols with id
    ...
    for (symbol in bnf) if (bnf.hasOwnProperty(symbol)) {
    ...
    // CATTAIL: variable name suffix with `_` all seemed to be mapping object
    ...
    /**
     * CATTAIL
     * @param {array|string} handle If string, converted into array and store in rhs.
     * If array, could either contain 2 element or 3 element with the following format:
     * [production body, (action)?, (precedence)?]
     */
    function buildProduction (handle) {
      ...
      if (handle.constructor === Array) {
        ...
        // store all symbols including terminals and non-terminals
        ...
        if (typeof handle[1] === 'string' || handle.length == 3) {
          ...
          /* CATTAIL
           * $$ => this.$ (possible has prefix ^'")
           * @0, @$ => this._$
           * $\d => $$[$0-number] (don't fully understand yet)
           */
      ...
      // CATTAIL: this.productions store array of Production
      productions.push(r);
      // CATTAIL: this.productions_ store every production and it's length as an array
  ...
  generator.warn = function warn () {
    ...
    Catpreter.print.call(console, args.join(""));
  };
  ...
  var generatorDebug = {
    trace: function trace () {
      Catpreter.print.apply(null, arguments);
    },
  ...
  /**
   * CATTAIL
   * Calculate nonterminal and production's nullable property.
   * Using a technique don't involved in recursive, but a additional loop wrapper.
   */
   ...
   * CATTAIL
   * @class Item
   * Define LR used item. Containing dot position and following set.
   */
  lrGeneratorMixin.Item = typal.construct({
     ...
     * @param {number} dot Dot position
     * @param {array} f Follow set
     * @param {} predecessor
     */
  ...
  /**
   * CATTAIL
   * Derived from Set, contain a inner implemented hash store all items.
   * Alter hash before other operations.
   * @class ItemSet
   */
  ...
  /**
   * CATTAIL
   * Calculate item closure.
   * @param {ItemSet} itemSet Inadequate closure set
   */
   ...
   * @param {string} symbol Terminal or nonterminal
   * @return {ItemSet} Newly created ItemSet
   */
  ...
  /**
   * CATTAIL
   * Using goto function calculate afterwords ItemSet.
   * ItemSet Maintain edges and predecessors bidirectional access between two ItemSet.
   * @param {string} symbol Symbol right after dot
   * @param {ItemSet} itemSet
   * @param {ItemSet} states Set of ItemSet
   * @param {number} stateNum current ItemSet index
   */
  ...
  /**
   * CATTAIL
   * @param {array.<ItemSet>} itemSets Array of ItemSet supplied by canonicalCollection()
   */
  lrGeneratorMixin.parseTable = function parseTable (itemSets) {
    ...
    itemSets.forEach(function (itemSet, k) {
      ...
      itemSet.reductions.forEach(function (item, j) {
        ...
        // CATTAIL: only terminals could be look ahead symbols.
   ...
   * @param {Production} production
   * @param {number} op
   * @param {array.<number, number>} reduce
   * @param {array.<number, number>} shift
   */
...
//    + "\n}"
  ...
  lrGeneratorMixin.generateModule = function generateModule (opt) {
    opt = typal.mix.call({}, this.options, opt);
    var moduleName = opt.moduleName || "parser";
    var out = "/* Catpreter generated parser */\n";
 ...
 */
  ...
  /**
   * CATTAIL
   * @param {string} input Handle will be parsed
   */
  ...
  Catpreter.Generator = function Catpreter_Generator (g, options) {
